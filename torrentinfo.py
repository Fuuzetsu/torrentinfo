...
import sys, getopt, os.path, re, time, unicodedata;
...
class String:
    # A static variable is really the easiest way to implement this without large changes
    asciionly = False
    ...
    def Dump ( self, formatter, tabchar, depth, newline=True):
        if self.isprintable:
            output = '%s%s' % ( tabchar * depth, self.value ) + ( '\n' if newline else '' );
            formatter.Format ( TextFormatter.NONE, output );
        else:
            output = '%s[%d UTF-8 Bytes]' % ( tabchar * depth, self.length ) + ( '\n' if newline else '' );
            formatter.Format ( TextFormatter.BRIGHT | TextFormatter.RED, output );
    ...
    def IsPrintable ( string ):
        ...
        isunicode = True if control_char_re.match(string.value) is None else False
...
class Dictionary:
    def __init__ ( self, string ):
        # Prefix char, then list of alternation string, object pairs until an 'e' is hit
        string.Get ( 1 );
        self.value = { };
        while string.Peek ( ) != 'e':
            key = String ( string );
...
def GetCommandlineArguments ( appname, arguments ):
    try:
        options, arguments = getopt.gnu_getopt ( arguments, 'hndbtfa', [ 'help', 'nocolour', 'dump', 'basic', 'top', 'files', 'ascii' ] );
...
def Basic ( formatter, torrent ):
    ...
    GetLine ( formatter, 'name       ', 'name', torrent [ 'info' ], format = TextFormatter.YELLOW | TextFormatter.DULL );
    GetLine ( formatter, 'tracker url', 'announce', torrent );
    GetLine ( formatter, 'created by ', 'created by', torrent );
    GetLine ( formatter, 'created on ', 'creation date', torrent, isdate = True );
...
def BasicFiles ( formatter, torrent ):
    ...
    if not 'files' in torrent [ 'info' ]:
        ...
        if numfiles > 1:
            ...
            reduce ( lambda x, y: x + y, lengths ).DumpAsSize ( formatter, '', 0 );
        else:
    ...
    try:
        ...
        for filename in filenames:
            try:
                ...
                sys.stderr.write ( 'Could not parse %s as a valid torrent file.\n' % filename );
